...
def cluster_embedding(adata, emb_key='X_emb', use_pca=True, job_name=''):
    if use_pca:
    ...
    plt.title(f"Embedding ({'PCA' if use_pca else 'UMAP'}) for {emb_key} ({job_name})")
    ...
    fig = plt.gcf()
    wandb.log({"Clusters using embedding": fig})