...
class PseudoBulkMappingStrategy(BaseMappingStrategy):
    ...
    """
    def __init__(self, name="pseudobulk", random_state=42, n_basal_samples=1, 
                 neighborhood_fraction=0.1, **kwargs):
    ...
    def register_split_indices(self, dataset, split, perturbed_indices, control_indices):
        ...
        if dataset.embed_key:
            ...
            X = np.vstack([
                dataset.fetch_gene_expression(idx).cpu().numpy()
        ...
        for i, cell_idx in enumerate(control_indices):
            ...
            if dataset.embed_key:
                expr_list = [
                    dataset.fetch_obsm_expression(n_idx, dataset.embed_key)
                    for n_idx in ctrl_neighbor_indices
                ]
            else:
                expr_list = [
                    dataset.fetch_gene_expression(n_idx) 
                    for n_idx in ctrl_neighbor_indices
                ]
        ...
        for pert_name in np.unique(pert_names):
            ...
            for idx, cell_idx in enumerate(pert_indices):
                ...
                if dataset.embed_key:
                    expr_list = [
                        dataset.fetch_obsm_expression(i, dataset.embed_key)
                        for i in pert_neighbor_indices
                    ]
                else:
                    expr_list = [
                        dataset.fetch_gene_expression(i) for i in pert_neighbor_indices
                    ]
    ...
    def get_mapped_expressions(self, dataset, split, idx) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if is_control:
            ...
            control_expr_list = [
                self.split_control_pseudobulks[split][ctrl_idx]
