...
class CentroidMappingStrategy(BaseMappingStrategy):
    ...
    def __init__(self, name="centroid", random_state=42, n_basal_samples=1, n_clusters=50, **kwargs):
        super().__init__(name, random_state, n_basal_samples)
        self.n_clusters = n_clusters

        # For each split, store the indices for centroid cells
        self.split_centroid_indices = {"train": [], "train_eval": [], "val": [], "test": []}
    ...
    def register_split_indices(
        ...
        centroid_indices, _ = pairwise_distances_argmin_min(kmeans.cluster_centers_, X)

        # Map back to original indices
        self.split_centroid_indices[split] = control_indices[centroid_indices]

        logger.info(f"[{dataset.name}] Split {split} has {len(centroid_indices)} centroid cells")
    ...
    def get_control_indices(self, dataset: "PerturbationDataset", split: str, perturbed_idx: int) -> np.ndarray:
        ...
        return self.rng.choice(centroid_pool, size=self.n_basal_samples, replace=True)
