...
class ClusteringMappingStrategy(BaseMappingStrategy):
    ...
    def __init__(self, name="clustering", random_state=42, n_basal_samples=1, n_clusters=50, **kwargs):
        super().__init__(name, random_state, n_basal_samples)
        self.n_clusters = n_clusters

        # For each split, store the indices for the largest cluster
        self.split_control_indices = {"train": {}, "train_eval": {}, "val": {}, "test": {}}
    ...
    def register_split_indices(
        ...
        logger.info(f"[{dataset.name}] Split {split} has {n_controls} control cells in largest cluster")
    ...
    def get_control_indices(self, dataset: "PerturbationDataset", split: str, perturbed_idx: int) -> np.ndarray:
        ...
        return self.rng.choice(control_pool, size=self.n_basal_samples, replace=True)
