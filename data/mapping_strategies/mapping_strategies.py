...
class BaseMappingStrategy(ABC):
    ...
    def __init__(self, name = None, random_state: int = 42, n_basal_samples: int = 1, stage: str = 'train', **kwargs):
        ...
        if dataset.embed_key:
            if is_control:
                ...
                ctrl_expr = torch.stack([
                    torch.tensor(dataset.fetch_obsm_expression(idx, dataset.embed_key))
            ...
            if is_control:
                ...
                control_indices = self.get_control_indices(dataset, split, perturbed_idx)
                ctrl_expr = torch.stack([
                    dataset.fetch_gene_expression(idx) for idx in control_indices
                ]).mean(0)
