...
class NearestNeighborMappingStrategy(BaseMappingStrategy):
    ...
    """
    def __init__(self, name="nearest", random_state=42, n_basal_samples=1, k_neighbors=10, pca_transform: PCATransform = None, **kwargs):
        ...
        self.split_neighbor_indices = {
            'train': None,
            'train_eval': None, 
            'val': None,
            'test': None
        }
        self.split_neighbor_weights = {
            'train': None,
            'train_eval': None,
            'val': None,
            'test': None
        }
        self.split_index_lookup = {
            'train': None,
            'train_eval': None,
            'val': None,
            'test': None
    ...
    def register_split_indices(
        ...
        self.split_neighbor_indices[split] = np.zeros(
            (n_cells, self.k_neighbors), dtype=np.int64
        )
        self.split_neighbor_weights[split] = np.zeros(
            (n_cells, self.k_neighbors), dtype=np.float32
        )
        ...
        for ct in np.unique(dataset.cell_type_categories):
            ...
            ct_mask = (dataset.h5_file[f'obs/{dataset.cell_type_key}/codes'][:] == ct_code) # this will just be the whole file?
