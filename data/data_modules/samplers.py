...
class PerturbationBatchSampler(Sampler):
    ...
    def __init__(self, dataset: "MetadataConcatDataset", batch_size: int, drop_last: bool = False):
        if hasattr(dataset, "data_source"):
            self.dataset = dataset.data_source # lightning sometimes tries to pass in dataset as a sequential sampler here...
        else:
    ...
    def __iter__(self) -> Iterator[List[int]]:
        ...
        grouped_indices_lists = {
            key: indices.copy() for key, indices in self.grouped_indices.items()
        }
