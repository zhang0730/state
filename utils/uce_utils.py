...
class UCEGenePredictor:
    ...
    def __init__(self,
        ...
        try:
            ...
            self.model.load_state_dict(
               torch.load(self.model_loc), strict=False)
    ...
    def get_reduced_embeds(self, genes):
        ...
        return self.model.gene_embedding_layer(
            torch.stack([self.protein_embeds[x] if x in self.protein_embeds else torch.zeros(5120) for x in genes]).to(self.device))
    ...
    def get_MLP_input_batched(self, cell_embeds, task_embeds):
        ...
        """
        A = task_embeds.unsqueeze(0).repeat(cell_embeds.size(0), 1,
                                            1)  # (batch_size, num_genes, embed_dim)
        B = cell_embeds.unsqueeze(1).repeat(1, task_embeds.size(0),
                                            1)  # (batch_size, num_genes, embed_dim)

        # Concatenating along the last dimension (embedding dimension)
        mlp_input = torch.cat([A, B],
                              dim=-1)  # (batch_size, num_genes, 2*embed_dim)
        return mlp_input

    def compute_gene_prob_group_batched(self, cell_embeds, genenames,
                                        batch_size=32):
        """
        ...
        for i in tqdm(range(0, cell_embeds.size(0), batch_size), total=int(
                np.ceil(cell_embeds.size(0) / batch_size))):
