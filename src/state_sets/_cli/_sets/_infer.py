...
def add_arguments_infer(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--output_dir", type=str, required=True, help="Path to the output_dir containing the config.yaml file that was saved during training.")
...
def run_sets_infer(args):
    ...
    logger.info(f"Running inference on {n_samples} samples in {n_batches} batches of size {batch_size} (model's cell_sentence_len)...")
