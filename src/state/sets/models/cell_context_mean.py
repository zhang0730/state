...
class CellContextPerturbationModel(PerturbationModel):
    """
    ...
    def on_fit_start(self):
        ...
        with torch.no_grad():
            ...
            # Now average deltas within cell contexts
            self.context_offsets = {}
            for context_name in celltype_pert_means.keys():
                # Gather deltas for this perturbation across cell types
                context_deltas = []
                for pert_name, pert_dict in celltype_pert_means[context_name].items():
                    if pert_name in pert_dict:
                        context_deltas.append(pert_dict[pert_name])

                if not context_deltas:
                    continue

                # Average the deltas
                self.context_mean_offsets[context_name] = torch.stack(context_deltas).mean(0)

            # Add zero offset for control
            self.context_mean_offsets[self.control_pert] = torch.zeros(self.output_dim)
            ...
            logger.info(f"GlobalSimpleSum: computed offsets for {len(self.context_mean_offsets)} contexts")
    ...
    def forward(self, batch: dict) -> torch.Tensor:
        ...
        For each sample in the batch, we do:
            prediction = global_basal + context_mean_offsets[p_name].
        """
        ...
        for i in range(B):
            ...
            offset_vec = self.context_mean_offsets.get(p_name, None)
