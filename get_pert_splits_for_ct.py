...
def split_perturbations(
    target_perts: Set[str], val_frac: float, test_frac: float, random_seed: int = 42
...
def main():
    ...
    parser.add_argument("--h5_files", type=str, nargs="+", required=True, help="List of paths to .h5 files to process")
    parser.add_argument("--cell_type", type=str, required=True, help="Target cell type for test set")
    parser.add_argument("--cell_type_key", type=str, default="cell_type", help="Key for cell type info in h5 files")
    parser.add_argument("--pert_key", type=str, default="perturbations", help="Key for perturbation data in h5 files")
    parser.add_argument(
        "--val_frac", type=float, default=0.1, help="Fraction of target cell type perturbations for validation set"
    )
    parser.add_argument(
        "--test_frac", type=float, default=0.1, help="Fraction of target cell type perturbations for test set"
    )
    ...
    filtered_perts = old_target_len - len(
        target_perts
    )  # for perturbations not found in training data, we just train on them
    ...
    train_perts, val_perts, test_perts = split_perturbations(
        target_perts, args.val_frac, args.test_frac, args.random_seed
    )
